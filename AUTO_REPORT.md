...
## 2026-06-20 - v12.0 인테리어디자이너+정원조경+도면뷰어+조명시뮬+비용계산기+한옥퍼즐+건축사연표+인증시스템 에디션

### 1차: 벤치마킹 / 분석

**경쟁앱 비교:**
- The Sims 4 (EA): 인테리어 디자인 풍부 + 정원 조경 있음 + 도면 뷰어 없음 + 조명 시뮬 있음 + 비용 계산 없음
- Home Design 3D (Anuman): 인테리어 가구 배치 + 정원 디자이너 + 2D 도면 + 조명 효과 + 비용 추정
- Minecraft Education: 퍼즐 모드 + 건축 연표 학습 + 인증/배지 시스템 + 정원 조경 없음

**열위점 분석 (v11 기준, 10개):**
1. ❌ 인테리어 가구 배치 없음 → ✅ 인테리어디자이너 10종 가구 Canvas 클릭배치 (침대/소파/책상/의자/장롱/서안/병풍/화로/TV/식탁)
2. ❌ 정원 조경 디자인 없음 → ✅ 정원조경디자이너 12종 10x10 Canvas 그리드 (소나무/대나무/벚나무/연꽃/잔디/돌/등불/정자/울타리/국화/철쭉/단풍)
3. ❌ 건축 도면 보기 없음 → ✅ 도면뷰어 4면 Canvas (평면도/정면도/측면도/배치도)
4. ❌ 조명 시뮬레이션 없음 → ✅ 조명시뮬레이터 6종 Canvas 시각화 (자연광/등불/촛불/형광등/LED/간접조명)
5. ❌ 건축 비용 상세 없음 → ✅ 비용계산기 6종 건물 Pie+Bar Canvas 차트 (재료/인건비/설비/설계/부대비용)
6. ❌ 구조 학습 퍼즐 없음 → ✅ 한옥구조퍼즐 3종 드래그배치 (기초구조/지붕구조/공간구조)
7. ❌ 건축 역사 연표 없음 → ✅ 건축사연표 12시대 Canvas 타임라인 (BC8000~미래)
8. ❌ 레벨 인증 시스템 없음 → ✅ 인증시스템 6단계 (견습생→기능사→기사→건축사→대목장→대스승)
9. ❌ 퀴즈 105문 한정 → ✅ +15문 추가 (105→120)
10. ❌ 업적 110개 한정 → ✅ +12개 추가 (110→122)

### 2차: 개발팀 실행

**v12_patch.js IIFE 모듈 (791줄, ~55KB):**

| 섹션 | 기능 | 상세 |
|------|------|------|
| CSS v12 | 패널 스타일링 | v12-panel, v12-box, v12-grid, v12-card, v12-canvas, v12-scroll-nav |
| HTML v12 | 8개 패널 DOM | interior, garden, blueprint, lighting, costcalc, puzzle, archtimeline, certification |
| SFX 12종 | Web Audio | interior_place, garden_plant, blueprint_view, light_change, cost_calc, puzzle_place, puzzle_correct, puzzle_wrong, timeline_view, cert_earn, feature_open12, achieve_v12 |
| 인테리어디자이너 | 10종 가구 | 침대/소파/책상/의자/장롱/서안/병풍/화로/TV/식탁 Canvas 클릭배치+삭제 |
| 정원조경디자이너 | 12종 | 소나무/대나무/벚나무/연꽃/잔디/돌/등불/정자/울타리/국화/철쭉/단풍 10x10 Canvas 그리드 |
| 도면뷰어 | 4면 | 평면도/정면도/측면도/배치도 Canvas 건축도면 렌더링 |
| 조명시뮬레이터 | 6종 | 자연광/등불/촛불/형광등/LED/간접조명 Canvas 시각화+밝기슬라이더 |
| 비용계산기 | 6종 건물 | 한옥/기와집/초가집/서원/정자/현대주택 Pie+Bar Canvas 차트 |
| 한옥구조퍼즐 | 3종 | 기초구조/지붕구조/공간구조 드래그배치+정답체크 |
| 건축사연표 | 12시대 | BC8000~미래 Canvas 타임라인 렌더링 |
| 인증시스템 | 6단계 | 견습생→기능사→기사→건축사→대목장→대스승 자동레벨업 |
| 퀴즈 +15 | 105→120 | 인테리어/정원/도면/조명/비용/퍼즐/연표/인증 관련 15문항 |
| 업적 +12 | 110→122 | 인테리어/정원/도면/조명/비용/퍼즐/연표/인증/퀴즈120/전체기능 관련 12업적 |
| 스크롤 네비 8 | 하단바 | 🛋인테리어/🌳정원/📐도면/💡조명/💰비용/🧩퍼즐/📜연표/🎓인증 |
| 키보드 8 | 단축키 | Shift+I/G/D/L/O/X/H/F |
...
|------|------|
| JS 문법 (node -c) | ✅ PASS |
| v12_patch.js 가드 | ✅ if(!window.__hbV12) 적용 |
| 개인정보 노출 | ✅ PASS (없음) |
...
- 변경 파일: v12_patch.js(신규 791줄), index.html, sw.js, manifest.json, AUTO_REPORT.md
- 총 기능: 건축모드6+풍수+마스터클래스12+대결AI+투어8+역사지도+에너지효율+건축일기+구조안전+재료실험실15+세계건축10+시공시뮬6+법규12+마을만들기+명장12+통계대시보드+BGM8+인테리어10+정원조경12+도면뷰어4+조명시뮬6+비용계산기6+한옥퍼즐3+건축사연표12+인증시스템6+퀴즈120+업적122
