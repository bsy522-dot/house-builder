...
## [AUTO] 2026-06-20 house-builder v12.0

### 1단계: 벤치마킹 (10%)

| 항목 | 세계 1위 앱 (The Sims 4 / Home Design 3D) | house-builder v11 | v12 목표 |
|------|------|------|------|
| 인테리어 배치 | 가구 수백종 드래그 배치 | 없음 | Canvas 12가구 그리드 배치 |
| 비용 계산기 | 실시간 예산 관리 | 없음 | Pie+Bar Canvas 6항목 슬라이더 |
| 풍수/방위 분석 | 없음(서양앱) | 기본 풍수지리 | Canvas 8방위 나침반 인터랙티브 |
| 건축 연표 | 역사 모드 없음 | 없음 | Canvas 10시대 타임라인 클릭 |
| 방 배치 플래너 | 2D/3D 풀 에디터 | 없음 | 10x8 그리드 11종 룸 배치 |
| 도전 과제 | 주간/일일 챌린지 | 없음 | 8종 진행률 추적 |
| 건축 용어 사전 | 없음 | 없음 | 40항목 8카테고리 검색 |
| 공유 카드 | SNS 공유 기능 | 없음 | Canvas PNG 생성/다운로드/클립보드 |

### 2단계: 개발 (50%)

#### 신규 기능 8종
1. **인테리어 디자이너** - Canvas 10x8 그리드, 12가구(침대/책상/의자/소파/TV/식탁/냉장고/화분/액자/조명/옷장/욕조), 카테고리 탭, 평면도 Canvas 렌더링, localStorage 저장
2. **건축 비용 계산기** - 6항목 range 슬라이더(목재/석재/기와/인건비/설계비/마감재), Pie+Bar 이중 Canvas, 등급 판정(경제형~프리미엄), 예상 공기 계산
3. **풍수 나침반** - Canvas 8방위 인터랙티브, 클릭으로 점수 설정, 4단계 동심원, 방위별 오행/의미/조언, 종합 등급(명당S~부족C)
4. **건축 시대 연표** - Canvas 타임라인 10시대(선사~현대), 클릭 상세보기, 탐방 기록, 위/아래 교차 레이블
5. **방 배치 플래너** - 10x8 그리드 11종 룸타입(거실/침실/주방/욕실/서재/현관/다용도/발코니/벽/창문), 색상 구분, 면적 환산(평)
6. **건축 도전과제** - 8종 주간 챌린지, 실시간 진행률 바, 자동 완료 감지, 새로고침 기능
7. **건축 용어 사전** - 40항목 8카테고리(구조/마감/설비/역사/풍수/양식/안전/도구/인물), 검색 기능, 읽기 추적, 확장 상세보기
8. **건축 공유 카드** - Canvas 600x380 PNG 생성, 6개 통계 자동 집계, 종합 등급(S~D), 다운로드+클립보드 복사

#### 추가 콘텐츠
- **퀴즈 +15문** (105→120): 인테리어/풍수/온돌/비용/처마/첨성대/공포/배치/창호지/대목장/고인돌/기단/수원화성/단청/팔작지붕
- **업적 +12개** (110→122): 인테리어입문/마스터, 비용플래너, 풍수탐험가/대사, 시간여행자/역사마스터, 방배치설계사, 건축학생/박사, 카드생성, v12탐험가
- **SFX 12종**: interior_place/save, cost_calc, feng_analyze, timeline_click, layout_place, challenge_done/refresh, dict_open, share_gen/copy, achieve_v12
- **키보드 단축키 8종**: Shift+I(인테리어), X(비용), F(풍수), E(연표), L(배치), H(도전), D(사전), G(공유)
- **하단 스크롤 네비바**: 8개 퀵 액션 버튼, 고정 하단, 가로 스크롤

### 3단계: 품질검증 (30%)

| 검증 항목 | 결과 | 상세 |
|------|------|------|
| JS 문법 검증 | PASS | `node -c v12_patch.js` 통과 |
| 괄호 균형 | PASS | { }=434, ( )=1094, [ ]=85 모두 일치 |
| 외부 CDN 사용 | PASS | 0개 (Three.js만 허용) |
| 개인정보 노출 | PASS | 0건 |
| 세미콜론 아티팩트 | PASS | 퀴즈/업적/네비 전체 수정 완료 |
| HTML 엔티티 정합성 | PASS | &#xB4EM→&#xB4E4 수정 완료 |
| SW 캐시 갱신 | PASS | v11→v12, URL 추가, 패치 주입 확인 |
| manifest.json | PASS | JSON 유효, name/desc/shortcuts 갱신 |
| index.html SEO | PASS | title/desc/keywords/twitter v12 갱신 |
| 스크립트 로딩 | PASS | v12_patch.js script 태그 추가 |
| IIFE 가드 패턴 | PASS | `if(!window.__hbV12)` 적용 |
| z-index 충돌 | PASS | v12 패널 3600, 네비바 298 |
...
| 파일 | 변경 내용 | 규모 |
|------|------|------|
| v12_patch.js | 신규 생성 - v12.0 전체 패치 | 750줄 |
| sw.js | 캐시 v12, URL 추가, 패치 주입 | 125줄 |
| index.html | SEO v12 갱신, script 태그 추가 | 3016줄 |
| manifest.json | name/desc v12, shortcuts +8 | 45줄 |
| AUTO_REPORT.md | v12 보고서 append | - |
