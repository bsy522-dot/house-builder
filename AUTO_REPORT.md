...
## 2026-06-20 - v12.0 평면도에디터+가구배치+건축비용계산+한옥해부도+건축연표+인테리어갤러리+건축자격시험+실내조명시뮬 에디션

### 1차: 벤치마킹 / 분석

**경쟁앱 비교:**
- The Sims 4 (EA): 평면도 에디터 있음 + 가구 배치 풍부 + 비용 시뮬 없음 + 건축사 자격 없음
- Home Design 3D (Anuman): 평면도 에디터 우수 + 가구 카탈로그 풍부 + 비용 견적 일부 + 조명 시뮬 있음
- Minecraft Education: 건축 교육 + 인테리어 제한적 + 비용 개념 없음 + 자격시험 없음

**열위점 분석 (v11 기준, 10개):**
1. ❌ 평면도 에디터 없음 → ✅ 평면도에디터 Canvas 격자 드래그 방+가구 배치 (800x600)
2. ❌ 가구 배치 시스템 없음 → ✅ 가구배치 12종 Canvas (침대/책상/식탁/소파/옷장/TV/서재/조명/화분/냉장고/세탁기/피아노)
3. ❌ 건축 비용 분석 없음 → ✅ 건축비용계산기 BarCanvas 면적×단가 6항목 (기초/골조/지붕/내외장/설비/인테리어)
4. ❌ 한옥 구조 해부도 없음 → ✅ 한옥해부도 Canvas 14부위 인터랙티브 (기단/초석/기둥/보/도리/서까래/추녀/용마루/처마/공포/창호/마루/온돌/담장)
5. ❌ 건축 역사 연표 없음 → ✅ 건축연표 Canvas 12시대 타임라인 (선사~현대)
6. ❌ 인테리어 스타일 갤러리 없음 → ✅ 인테리어스타일갤러리 12종 (모던/미니멀/스칸디/재패니즈/한옥/인더스트리얼/보헤미안/클래식/러스틱/아르데코/팝아트/와비사비)
7. ❌ 건축 자격시험 없음 → ✅ 건축자격시험 20문항 4지선다 (구조/시공/법규/설비/환경)
8. ❌ 실내 조명 시뮬 없음 → ✅ 실내조명시뮬 Canvas 3종 광원 (자연광/백열등/LED) 밝기/색온도 실시간
9. ❌ 퀴즈 105문 한계 → ✅ 퀴즈 15문 추가 (105→120)
10. ❌ 업적 110개 한계 → ✅ 업적 12개 추가 (110→122)

### 2차: 개발팀 실행

**v12_patch.js IIFE 모듈 (828줄, ~57KB):**

| 섹션 | 기능 | 상세 |
|------|------|------|
| CSS v12 | 패널 스타일링 | v12-panel, v12-box, v12-grid, v12-card, v12-tab 등 |
| HTML v12 | 8개 패널 DOM | floorplan, furniture, costcalc, anatomy, timeline, interior, license, lighting |
| SFX 12종 | Web Audio | floor_draw, furniture_place, cost_calc, anatomy_click, timeline_scroll, interior_pick, license_correct, license_wrong, light_toggle, light_adjust, quiz_v12, achieve_v12 |
| 평면도에디터 | Canvas 800x600 | 격자 기반 방 드래그 생성, 가구 아이콘 배치, 초기화/저장 |
| 가구배치 | 12종 가구 | 침대/책상/식탁/소파/옷장/TV/서재/조명/화분/냉장고/세탁기/피아노 Canvas 렌더링 |
| 건축비용계산기 | BarCanvas | 면적 슬라이더(10~300평), 6항목 단가, 총합 실시간 Bar차트 |
| 한옥해부도 | Canvas | 14부위 인터랙티브 (클릭→설명 팝업), 색상 하이라이트 |
| 건축연표 | Canvas | 12시대 타임라인 (선사/삼국/통일신라/고려/조선초/조선중/조선후/개화기/일제/해방후/현대/미래) |
| 인테리어갤러리 | 12종 스타일 | 모던/미니멀/스칸디/재패니즈/한옥/인더스트리얼/보헤미안/클래식/러스틱/아르데코/팝아트/와비사비 |
| 건축자격시험 | 20문 4지선다 | 5카테고리(구조/시공/법규/설비/환경), 점수+합격판정(80점↑) |
| 실내조명시뮬 | Canvas | 3종 광원, 밝기 슬라이더, 색온도 슬라이더, 실시간 렌더링 |
| 퀴즈 +15 | 105→120 | 평면도/가구/비용/해부도/연표/인테리어/자격/조명 관련 15문항 |
| 업적 +12 | 110→122 | 평면도/가구/비용/해부도/연표/인테리어/자격/조명/퀴즈115/전체 관련 12업적 |
| FAB 8 | 하단스크롤바 | 📐평면도/🪑가구/💰비용/🏛해부도/📜연표/🎨인테리어/📝자격/💡조명 |
| 키보드 8 | 단축키 | Shift+F/U/O/A/E/I/L/G |
...
|------|------|
| JS 문법 (node -c) | ✅ PASS |
| 괄호 균형 | ✅ PASS ({377/377} [87/87]) |
| HTML div 균형 | ✅ PASS (43/43) |
| 개인정보 노출 | ✅ PASS (없음) |
...
- 커밋: `[AUTO] 2026-06-20 house-builder v12.0`
- 변경 파일: v12_patch.js(신규), index.html, sw.js, manifest.json, AUTO_REPORT.md
- 총 기능: 건축모드6+풍수+마스터클래스12+대결AI+투어8+역사지도+에너지효율+건축일기+구조안전+재료실험실15+세계건축10+시공시뮬6+법규12+마을만들기+명장12+통계대시보드+BGM8+평면도에디터+가구배치12+건축비용계산+한옥해부도14+건축연표12+인테리어갤러리12+건축자격시험20+실내조명시뮬+퀴즈120+업적122
