...
## 2026-05-09 - v4.0 서원 에디션 + 대규모 콘텐츠 확장

### 1차: 벤치마킹 / 분석

**경쟁앱 비교:**
- The Sims (EA): 5+ 건축 유형, 조경 요소, 비교 기능, 풍부한 인테리어
- Home Design 3D (Anuman): 건축 비교 모달, 상세 통계, 다양한 모드
- Minecraft 교육판: 다수 건축 모드, 30+ 업적, 교육 퀴즈 풍부

**v3.0 대비 열위점 (v4.0에서 해결):**
| # | 열위점 | v4.0 해결 |
|---|--------|----------|
| 1 | 건축 모드 4종만 | 5번째 모드: 서원(書院) 7단계 추가 |
| 2 | 퀴즈 모드당 3문제 (12문제) | 모드당 6문제로 확장 (총 30문제) |
| 3 | 비교 기능 alert() 사용 | 정교한 모달 UI로 전면 교체 (8항목 비교표) |
| 4 | 업적 16개만 | 22개로 확대 (+6: 서원학자/건축박사/연속정답왕/새벽건축가/조경감상가/비교달인) |
| 5 | 조경 요소 없음 | 연못+석등+돌길 3D 조경 추가 |
| 6 | 야간 앰비언트 부족 | 귀뚜라미 사운드 추가 (야간 전용) |
| 7 | 건축 상식 부족 | 서원 7개 팩트 추가 (총 32개) |
| 8 | 사운드 분류 미흡 | 서원/기와집 부품별 사운드 매핑 확장 |
| 9 | 모드 5종 비교 불가 | 5개 모드별 8항목 비교 데이터 완비 |
| 10 | 완성 추적 4/4 | 5/5 추적 체계로 확대 |

**v3.0 대비 우위점 (유지+강화):**
- 한국 전통 건축 특화 교육 (5종 모드: 한옥/초가/기와/현대/서원)
- 단계별 건축 과정 학습 + 상세 비교 모달
- 30문제 퀴즈로 깊이 있는 학습
- 유네스코 세계유산(서원) 교육 콘텐츠
...
**[프론트엔드 / UI-UX]**
- 5번째 모드 카드 추가 (서원 📚, 블루 보더 글로우 `#6498c8`)
- 건축 비교 모달 (`#compareModal`) — 그리드 레이아웃 8항목 비교표
- 타임라인 CSS 추가 (`.timeline-list`, `.timeline-item`)
- 모바일 반응형: 비교 모달 `max-width:600px` 스크롤 대응

**[백엔드 / 게임 로직]**
- 서원 모드 (gameMode='seowon') 전체 게임 루프 통합
- `toggleCompare()` 완전 재작성: alert() → 모달 (5개 모드별 비교 데이터)
- `closeCompare()` 함수 신규
- `checkMasterBuilder()` 5모드 체크 (all_modes 업적)
- `updateProgressBadge()` 5/5 추적
- `startAmbient()` 야간 귀뚜라미 사운드 (4000-4400Hz 트리플 chirp)
- `playBuildSfx()` 서원/기와집 부품 사운드 매핑 확장

**[콘텐츠 제작]**
- **서원(書院) 모드 신규 (7단계):**
  1. 강학 기단 — 다듬돌/자연석
  2. 기둥 — 소나무/느티나무
  3. 보와 도리 — 소나무/참나무 (5량 구조)
  4. 맞배지붕 — 회기와/청기와 (합각벽 포함)
  5. 강학당 — 소나무마루/참나무마루 (스승 책상+학생 책상 6개)
  6. 동재와 서재 — 흙벽+창호지/판벽+창호지 (좌우 대칭 기숙사)
  7. 사당 — 소나무/느티나무 (위패+제단+기와지붕)
- **퀴즈 18문제 추가** (기존 12→30문제):
  - 서원 6문제: 전학후묘/동재/맞배지붕/유네스코9곳/소수서원/성리학
  - 한옥 +3: 칸/서까래/한지
  - 현대 +3: 내력벽/패시브하우스/필로티
  - 초가 +3: 매년교체/토방/화재위험
  - 기와 +3: 주심포식/안채/막새
- **서원 팩트 카드 7개** (강학기단~사당 건축상식)
- **업적 6개 추가**: 서원학자/건축박사/연속정답왕/새벽건축가/조경감상가/비교달인

**[오디오 엔진]**
- 야간 귀뚜라미 앰비언트 (4000-4400Hz, 50ms 간격 트리플 chirp, 3-5초 주기)
- 서원 부품 사운드 매핑: 기단→stone, 기둥/보/강학당/사당→wood, 지붕→tile

**[비주얼 / 3D]**
- 서원 7개 파트 3D 모델링:
  - 강학 기단: 넓은 기단(10x8) + 계단
  - 기둥: 10개 원기둥 배치
  - 보와 도리: 5량 구조 + 종도리 + 대공 3개
  - 맞배지붕: 앞뒤 경사면 + 삼각 합각벽(BufferGeometry) + 기와선 14줄
  - 강학당: 넓은 마루 + 스승 책상 1개 + 학생 책상 6개
  - 동재/서재: 좌우 대칭 건물 쌍 + 창호 + 미니 지붕
  - 사당: 독립 건물(z=-5) + 위패 + 제단 + 4기둥 + 기와지붕
- 조경 요소 3종:
  - 연못: CircleGeometry r=2 반투명 수면 (8,-6)
  - 석등: 4단 적층 (받침+기둥+화창+보개+보주) (-8,5)
  - 돌길: 8개 원형 징검돌 (z=6.5~12.9)

**[데이터]**
- SEOWON_STEPS: 7단계, 각 2개 재료 선택지
- QUIZZES: 5모드 × 6문제 = 30문제
- FACTS: +7개 서원 팩트 (총 32개)
- ACHIEVEMENTS: 16→22개 (+6개)
- 비교 데이터: 5모드 × 8항목 = 40개 비교 포인트
...
**코드 리뷰:**
- JavaScript 문법: Node.js `new Function()` 파싱 성공 — PASS
- HTML 태그 밸런스: div 109/109, button 27/27 — PASS
- 외부 URL: Three.js CDN + SVG namespace만 사용 — PASS (규칙 준수)
- 개인정보: 0건 — PASS
- SEOWON_STEPS 7단계 정의 확인 — PASS
- buildSeowonPart 7 case 전부 구현 — PASS
- 서원 퀴즈 6문제 + 기존 모드 각 +3문제 — PASS
- 서원 팩트 7개 — PASS
- 업적 22개 — PASS
- 비교 모달 5개 모드 데이터 — PASS
- onclick 핸들러 32개 전부 정의됨 — PASS
- startGame/buildStep/afterBuild 서원 분기 — PASS

**성능 체크:**
- 서원 3D 모델 geometry dispose 기존 로직으로 커버됨
- 야간 귀뚜라미 setInterval은 muted 체크로 불필요 실행 방지
- 조경 요소(연못/석등/돌길)는 정적 메쉬로 성능 영향 최소
...
**파일 변경:**
- `index.html`: 2699줄 → 3081줄 (+382줄, +14%), 121KB → 145KB (+20%)
- `sw.js`: v3 → v4 (캐시 버전 업데이트)
- `manifest.json`: 서원 모드 반영 + shortcuts 추가
- `AUTO_REPORT.md`: v4.0 보고서 추가

**투입 요약:**
- 벤치마킹 (10%): The Sims/Home Design 3D/Minecraft 대비 10개 열위점
- 개발 (50%): 서원7단계+퀴즈30문제+비교모달+업적22+조경3종+야간앰비언트
- 품질 (30%): 구문검증+태그밸런스+ID참조+외부URL+성능체크+전체통합확인
- 마무리 (10%): 보고서+커밋
